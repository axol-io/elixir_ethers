defmodule Ethers.MEVTest do
  ...
  describe "create_bundle/2" do
    test "creates a bundle with valid parameters" do
      ...
      assert {:ok, bundle} = MEV.create_bundle(transactions, block_number: 12345)
    ...
    test "creates a bundle with optional parameters" do
      ...
      assert {:ok, bundle} = MEV.create_bundle(transactions,
        block_number: 12345,
    ...
    test "returns error for empty transactions" do
      assert {:error, :empty_bundle} = MEV.create_bundle([], block_number: 12345)
    end
  end
  ...
  describe "create_bundle!/2" do
    test "creates a bundle with valid parameters" do
      ...
      bundle = MEV.create_bundle!(transactions, block_number: 12345)
      assert %Bundle{} = bundle
      assert bundle.transactions == transactions
    end
  ...
  describe "pipeline functions" do
    test "bundle/2 creates a bundle" do
      ...
      bundle = MEV.bundle(transactions, block_number: 12345)
      assert %Bundle{} = bundle
      assert bundle.transactions == transactions
      assert bundle.block_number == 12345
    end
    ...
    test "with_timestamp_range/3 sets timestamps" do
      bundle = 
        ["0x" <> String.duplicate("ab", 32)]
        |> MEV.bundle(block_number: 12345)
        |> MEV.with_timestamp_range(1234567890, 1234567900)
  ...
  describe "provider functions without configured provider" do
    ...
    test "get_bundle_status/3 returns error when no provider configured" do
      assert {:error, :no_provider_configured} = MEV.get_bundle_status("0xhash", 12345)
    end
