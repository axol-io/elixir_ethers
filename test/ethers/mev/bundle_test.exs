defmodule Ethers.MEV.BundleTest do
  ...
  describe "new/1" do
    test "creates a bundle with valid parameters" do
      ...
      assert {:ok, bundle} = Bundle.new(%{
        transactions: transactions,
        block_number: 12345
      })
      
      assert bundle.transactions == transactions
      assert bundle.block_number == 12345
    ...
    test "creates a bundle with all optional parameters" do
      ...
      assert bundle.min_timestamp == 1234567890
      assert bundle.max_timestamp == 1234567900
    ...
    test "returns error for empty transaction list" do
      assert {:error, :empty_bundle} = Bundle.new(%{
        transactions: [],
        block_number: 12345
      })
    end
    ...
    test "returns error for invalid block number" do
      ...
      assert {:error, :invalid_block_number} = Bundle.new(%{
        transactions: transactions,
        block_number: 0
      })
    ...
    test "returns error for invalid timestamp range" do
      ...
      assert {:error, :invalid_timestamp_range} = Bundle.new(%{
        transactions: transactions,
        block_number: 12345,
        min_timestamp: 1234567900,
  ...
  describe "set_block_target/3" do
    test "sets the block target" do
      ...
      updated_bundle = Bundle.set_block_target(bundle, 99999)
      assert updated_bundle.block_number == 99999
    end

    test "sets block target with max block" do
      bundle = create_test_bundle()
      
      updated_bundle = Bundle.set_block_target(bundle, 99999, 100010)
      assert updated_bundle.block_number == 99999
      assert Map.get(updated_bundle, :max_block) == 100010
    end
  end
  ...
  describe "set_timing_constraints/3" do
    test "sets timing constraints" do
      ...
      updated_bundle = Bundle.set_timing_constraints(bundle, 1234567890, 1234567900)
      assert updated_bundle.min_timestamp == 1234567890
      assert updated_bundle.max_timestamp == 1234567900
    end

    test "allows nil timestamps" do
      bundle = create_test_bundle()
      
      updated_bundle = Bundle.set_timing_constraints(bundle, nil, 1234567900)
  ...
  describe "encode/1" do
    ...
    test "encodes a bundle with all fields" do
      ...
      assert encoded.blockNumber == "0x3039"
      assert encoded.minTimestamp == 1234567890
      assert encoded.maxTimestamp == 1234567900
